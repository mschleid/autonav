...
import { tagsGetAll, tagUpdate, tagCreate, tagDelete } from '../services/apiService';
...
function Tags() {
    ...
    async function completeModal(e) {
        ...
        if (modalCreateNew) {
            try {
                tagCreate(name, address).then((data) => {
            ...
            try {
                tagUpdate(selectedRow.id, name, address).then((data) => {
                    setSuccessUpdate(true);
                    setTagModalOpen(false);
                    loadData();
                });
    ...
    async function deleteTag(e) {
        ...
        try {
            tagDelete(selectedRow.id).then((data) => {
    ...
    return (
        <Sheet>
            ...
            >
                Tag succesfully {modalCreateNew ? "created" : "updated"}.
            </Snackbar>
            <Snackbar
                autoHideDuration={6000}
                color="success"
            ...
            <Modal open={tagModalOpen} onClose={() => setTagModalOpen(false)}>
                <ModalDialog>
                    ...
                    <DialogTitle>{modalCreateNew ? 'Add Tag' : 'Edit Tag'}</DialogTitle>
                    <form
                                ...
                                <Grid container spacing={2} columns={2} sx={{ flexGrow: 1, alignItems: 'right', justifyItems: 'right' }}>
                                    {
                                        (modalCreateNew) ? "" :
                                            <Grid size={6}>
                                                <Button color="danger" variant="outlined" onClick={deleteTag}>Delete Tag</Button>
                                            </Grid>
                                    }
                                    <Grid size={'grow'}>
                                        <Button type="submit">{modalCreateNew ? "Create Tag" : "Save Changes"}</Button>
                                    </Grid>
                                </Grid>
                            </Sheet>
            ...
            {/* Begin Content */}
            <Typography level="h2">Tags</Typography>
            <br />
            {/* Begin Table */}
            <Sheet
                variant="outlined"
